...
exports.register =  async (req, res, next) => {
    ...
    try{
        ...
        const medical_center = req.body.medical_center;
        ...
        if(!appointment_id || !date || !time || !medical_center || !doctor || !patient || !description || !username){
...
exports.search_with_username = async (req, res, next) => {
    console.log("Try to get all appointments with username");
    try{
        ...
        const result = await AppointmentModel.find({username: username}).exec();
        console.log("Appointments retrieved from database successfully!");
        res.status(201).send(result);
    }
    catch(error){
...
exports.search_with_m_center = async (req, res, next) => {
    console.log("Try to get all appointments with medical center");
    try{
        const m_center = req.params.m_center;
        if(!m_center){
            throw createHttpError.BadRequest('Missing required fields');
        }

        const result = await AppointmentModel.find({m_center: m_center}).exec();
        console.log("Appointments retrieved from database successfully!");
        res.status(201).send(result);
    }
    catch(error){
